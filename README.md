...
※ 모든 모듈은 MVC 패턴에 맞춰 Controller / Service / Mapper / XML / VO 동일한 구조로 구성
...
패키지 구조 개선
현재 테이블 단위로 나뉘어 있어 가독성이 떨어져
→ React 리팩토링과 함께 도메인 단위 패키지 구조로 재정비 예정

재고 이력 추적 강화
단일 row 구조의 한계를 보완하기 위해
→ inventory_history 테이블 추가 계획

배포/CI 자동화
Docker + GitHub Actions 기반 CI/CD 구축 예정
